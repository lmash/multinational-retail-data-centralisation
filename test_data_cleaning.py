    ...
    df = pd.DataFrame(data=[
        ...
        [
            ...
            '4 Whatever Street\nChicago'
                      ...
                      columns=[
                          'index', 'last_name', 'country_code', 'country',
                          'date_of_birth', 'join_date', 'address'
                      ])
    cleaned_df = user_cleaner.clean_user_data(df=df)
...
def test_clean_card_data_remove_rows_with_invalid_card_provider(card_cleaner):
    ...
    df = pd.DataFrame(data=[
        ...
        [
            '30060773296198', '09/32', 'American Express', '2015-11-25',
            np.nan, np.nan
        ],
    ],
                      columns=[
                          'card_number', 'expiry_date', 'card_provider',
                          'date_payment_confirmed', 'card_number expiry_date',
                          'Unnamed: 0'
                      ])
...
def test_clean_product_data_removes_rows_with_invalid_category(
    ...
    df = pd.DataFrame(data=[
        ...
        [
            '2', 'Dog food', '£4.49', '12 x 100g', 'pets', '2439834307647',
            '1995-06-25', '5ec5a431-7330-4d9e-bf3c-702fc85f6efe',
            'Still_avaliable', 'd4-9698287C'
        ],
    ],
                      columns=[
                          'index', 'product_name', 'product_price', 'weight',
                          'category', 'EAN', 'date_added', 'uuid', 'removed',
                          'product_code'
                      ])
...
def test_convert_product_weights_with_a_number_of_items(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '2 x 200g'}])
...
def test_convert_product_weights_with_ml(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '600ml'}])
    cleaned_df = product_cleaner.convert_product_weights(df=df)
...
def test_convert_product_weights_with_kg(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '0.6kg'}])
    cleaned_df = product_cleaner.convert_product_weights(df=df)
    assert cleaned_df['weight'][0] == 0.6
...
def test_convert_product_weights_with_oz(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '16oz'}])
    cleaned_df = product_cleaner.convert_product_weights(df=df)
...
def test_product_weight_remove_invalid_characters(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '16g .'}])
    cleaned_df = product_cleaner.convert_product_weights(df=df)
...
def test_convert_product_weights_with_g(product_cleaner):
    ...
    df = pd.DataFrame(data=[{'index': '0', 'weight': '11.6g'}])
    cleaned_df = product_cleaner.convert_product_weights(df=df)
...
def test_drop_columns(order_cleaner):
    ...
    cleaned_df = order_cleaner._drop_columns(
